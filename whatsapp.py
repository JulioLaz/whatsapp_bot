...
def buscar_respuesta():
	   # minimizar whatsapp
	posicion = pt.locateCenterOnScreen('imagenes/minimizar.png', grayscale=False, confidence=.9)
	if posicion is not None:
		x = int(posicion[0])*pct_x
		y = int(posicion[1])*pct_y
		...
		# recargar pagina openai
	posicion = pt.locateCenterOnScreen('imagenes/chat.png', grayscale=False, confidence=.9)
	if posicion is not None:
		x = int(posicion[0])*pct_x
		y = int(posicion[1])*pct_y
		pt.moveTo(x,y, duration=.05)
		...
		# pegar texto
	posicion = pt.locateCenterOnScreen('imagenes/send.png', grayscale=False, confidence=.9)
	if posicion is not None:
		x = (int(posicion[0])*pct_x)-100
		y = int(posicion[1])*pct_y
		pt.moveTo(x,y, duration=.05)
		sleep(1)
		pt.click()
		pt.hotkey("ctrl", "v")
		pt.write(". utiliza como max 50 palabras")
		sleep(3)

		# pegar enviar
	posicion = pt.locateCenterOnScreen('imagenes/send_verde.png', grayscale=False, confidence=.9)
	if posicion is not None:
		x = int(posicion[0])*pct_x
		y = int(posicion[1])*pct_y
		pt.moveTo(x,y, duration=.05)
		...
		# copy respuesta: 
	posicion = pt.locateCenterOnScreen('imagenes/copy.png', grayscale=False, confidence=.9)
	if posicion is not None:
		x = int(posicion[0])*pct_x
		y = int(posicion[1])*pct_y
		pt.moveTo(x,y, duration=.05)
...
def ejecutar_busqueda():
    if ejecutar_proceso:
        ...
        ventana.after(5000, ejecutar_busqueda)  # Llama a esta función nuevamente después de 7 segundos
